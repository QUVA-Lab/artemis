...
class ExperimentRecord(object):
    ...
    def args_valid(self, last_run_args=None, current_args=None, ignore_valid_keys=()):
        ...
        last_run_args = {k:v for k,v in dict(last_run_args).items() if k not in ignore_valid_keys}
        current_args = {k:v for k,v in dict(current_args).items() if k not in ignore_valid_keys}
        try:
