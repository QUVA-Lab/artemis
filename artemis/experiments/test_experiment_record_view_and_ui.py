...
my_experiment.add_variant('a2', a=2)
...
def test_experiments_function_additions():
    ...
    with experiment_testing_context():
        ...
        with CaptureStdOut() as cap:
            display_experiment_record(my_experiment.get_latest_record())
        assert cap.read() == '3aaa\n'

        with CaptureStdOut() as cap:
            compare_experiment_results([my_experiment, my_experiment.get_variant('a2')])
        assert cap.read() == 'my_experiment: 3, my_experiment.a2: 4\n'
...
def test_experiment_function_ui():
    ...
    with experiment_testing_context():
        for existing_record in my_experiment.get_variant_records(flat=True):
