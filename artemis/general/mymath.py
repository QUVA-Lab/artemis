...
def truncated_normal_moments(mu, sigma_sq):
    """
    A multivariate normal distribution is truncated at zero.  Find the mean and variance of the result.

    We use reference the paper:
    ON THE FIRST AND SECOND MOMENTS OF THE TRUNCATED MULTI-NORMAL DISTRIBUTION AND A SIMPLE ESTIMATOR
    Lung-Fei Lee, October 1979
    http://conservancy.umn.edu/bitstream/handle/11299/55008/1/1979-122.pdf
    The answer is in equation (8)

    :param mu: A length-N vector indicating the standard deviation
    :param sigma_sq: A shape (N, N) symmetric covariance matrix.
    :return: trunc_mu, trunc_sigma_sq
    """
    # https://en.wikipedia.org/wiki/Truncated_normal_distribution
    # See theorem 1 here: http://www.jstor.org/stable/pdf/1914214.pdf

    marginal_sigmas = np.sqrt(np.diagonal(sigma_sq))

    # mean: mu + sigma * (norm.pdf(a, mu, sigma) - norm.pdf(b, mu, sigma)) / (norm.cdf(b, mu, sigma)-norm.cdf(a, mu, sigma))
    # Where a=0, b=inf, becomes
    # mu + sigma * (norm.pdf(0, mu, sigma)-0)/(1-.5)

    inv_sigma = np.linalg.inv(sigma_sq)

    alpha = -mu/marginal_sigmas

    # alpha = np.sqrt(mu.dot(inv_sigma).dot())


    truncated_mean = (np.sqrt(sigma_sq)*(norm.pdf(0)-1)).sum(axis=1) + mu

    # truncated_mean = mu + marginal_sigmas*norm.pdf(alpha, loc=0, scale=1)/(1-norm.cdf(alpha))
    #
    # truncated_sigma_sq = 1./(np.linalg.inv(sigma_sq)) + (mu*truncated_mean)[:, None]
    #

    raise NotImplementedError("eeehhhm.  We have an equation relating the variance to the mean, but can't get the equation for mthe mean..")
